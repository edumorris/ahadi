...
<div class="product_meta">
    ...
    if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) && auxin_is_true( $show_sku ) ) : ?>
        <div class="auxshp-sku-wrapper auxshp-meta-section">
            <span class="sku-lable auxshp-label label-default"><?php _e( 'SKU', 'woocommerce' ); ?></span> 
            <span class="sku-value" itemprop="sku"><?php echo ( $sku = $product->get_sku() ) ? $sku : __( 'N/A', 'woocommerce' ); ?></span>
        </div>
    <?php endif; ?>
    ...
    <div class="auxshp-cats-wrapper auxshp-meta-section">
        <?php echo wc_get_product_category_list( $product->get_id(), ', ', '<span class="auxshp-label label-default auxshp-product-cats posted_in">' . _n( 'Category', 'Categories', $cat_count, 'woocommerce' ) . '</span><span class="auxshp-meta-terms">', '</span>' ); ?>
    </div>
    <?php endif; ?>
    <?php
    ...
    <div class="auxshp-tags-wrapper auxshp-meta-section">
        <?php echo wc_get_product_tag_list( $product->get_id(), ', ', '<span class="auxshp-label label-default auxshp-product-tags tagged_as">' . _n( 'Tag', 'Tags', $tag_count, 'woocommerce' ) . '</span><span class="auxshp-meta-terms">', '</span>' ); ?>
    </div>
    <?php endif; ?>
