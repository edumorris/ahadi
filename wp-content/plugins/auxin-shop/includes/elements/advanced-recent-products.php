...
function auxin_get_the_advance_recent_products_master_array( $master_array ) {
    ...
    $master_array['aux_advance_recent_product'] = array(
        'name'                    => __("Advance Recent Products", 'auxin-shop'  ),
...
function auxin_widget_the_advance_recent_products_callback( $atts, $shortcode_content = null ){
    ...
    if ( $result['parsed_atts']['show_filters'] ) {
        ...
        if ( $terms ) {
            ...
            foreach ( $terms as $term ) {
                if( (! is_array( $result['parsed_atts']['cat']) ) && !( empty( $result['parsed_atts']['cat'] ) || $result['parsed_atts']['cat'] == " " ) ) {
                    $result['parsed_atts']['cat'] = array( $result['parsed_atts']['cat'] );
                }

                if ( ( empty( $result['parsed_atts']['cat'] ) || $result['parsed_atts']['cat'] == " " || ( is_array( $result['parsed_atts']['cat'] ) && in_array( " ", $result['parsed_atts']['cat'] ) ) ) || in_array( $term->term_id, $result['parsed_atts']['cat'] ) ) {
                    $list_output .= '<li data-filter="'.$term->slug.'"><a href="#"><span data-select="'.$term->name.'">'.$term->name.'</span></a></li>';
                }
            }

            $output_classes  = 'aux-ajax-filters aux-filters ' . $result['parsed_atts']['filter_style'] . ' ' . $result['parsed_atts']['filter_align'] . ' ';
            $output_classes .= 'aux-dropdown-filter' != $result['parsed_atts']['filter_style'] ? 'aux-togglable ':                         '';
